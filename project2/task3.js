...
function getSelected(event) {
  ...
  if (!is_question_answered.includes(0)) {
    ...
    results_button.style.display='flex'
    results_button.style.backgroundColor = "#093009";
  }
}
...
async function getResults(event) {
  if (results_button.innerText === "Check Answers" && !event.target.disabled) {
    ...
    for (var i = 0; i < selected_answers.length; i++) {
      let para = document.createElement("p");
      temp_pt=selected_answers[i].isCorrect ? 1 : 0;
      
      para.innerHTML = `${i + 1}. You selected <strong style="color:${answer_colors[temp_pt]};">${
        selected_answers[i].answer
      }</strong>, the correct answer is <strong style="color:green;">${
        series_quotes[i].author
      }</strong>: Points ${selected_answers[i].isCorrect ? 1 : 0}`;
      resultsDiv.append(para);
      points += temp_pt;
    }
    h2.innerText+=`${points}/${no_of_quotes}`
    /*let para = document.createElement("p");
